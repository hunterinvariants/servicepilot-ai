...
def audit(db: Session, event_type: str, detail: dict, ticket_id: str | None = None, actor: str = "system"):
    db.add(AuditEvent(ticket_id=ticket_id, actor=actor, event_type=event_type, detail=detail))


def suggest_technician(db: Session, category: str) -> Technician | None:
    candidates = db.scalars(select(Technician).where(Technician.available.is_(True))).all()
    return next((tech for tech in candidates if category in tech.skills), candidates[0] if candidates else None)


def create_intake(db: Session, request: ServiceRequest) -> Ticket:
    provider = get_provider(get_settings())
    result, metrics = provider.classify(request)
    customer = db.scalar(select(Customer).where(Customer.email == request.email))
    if customer is None:
        customer = Customer(name=request.name, email=request.email, phone=request.phone, address=request.address)
        db.add(customer)
        db.flush()
    tech = suggest_technician(db, result.category)
    ...
    ticket = Ticket(reference=reference, customer_id=customer.id, technician_id=tech.id if tech else None,
        source=request.source, raw_message=request.message, category=result.category, summary=result.summary,
        urgency=result.urgency, confidence=result.confidence, requested_window=result.requested_window,
        proposed_appointment=proposed, quote_amount=result.quote_amount, response_draft=result.response_draft,
        risk_flags=result.risk_flags, status=TicketStatus.ESCALATED if risky else TicketStatus.PENDING_APPROVAL)
    db.add(ticket)
    db.flush()
    reason = "Risk or uncertainty requires review" if risky else "Approve quote, technician, appointment, and response"
    db.add(Approval(ticket_id=ticket.id, action="confirm_service_plan", reason=reason))
    db.add(MetricEvent(provider=provider.name, operation="classify", latency_ms=metrics["latency_ms"],
                       input_tokens=metrics["input_tokens"], output_tokens=metrics["output_tokens"]))
    audit(db, "ai_intake_completed", {"provider": provider.name, "classification": result.model_dump(),
          "technician": tech.name if tech else None, "knowledge": retrieve(request.message)}, ticket.id, "ai-agent")
    db.commit()
    db.refresh(ticket)
    return ticket
...
def decide_approval(db: Session, approval: Approval, approve: bool, decided_by: str, note: str = "") -> Ticket:
    ...
    audit(db, "service_plan_approved" if approve else "service_plan_rejected",
...
def dispatch_webhook(ticket_id: str):
    ...
    if settings.webhook_url:
        httpx.post(settings.webhook_url, json={"event": "ticket.updated", "ticket_id": ticket_id}, timeout=10).raise_for_status()
